	...
	m_AOConstantBuffer(DeviceAccessor::GetInstance()->GetDevice())
{
	...
	auto vsBlob = DX::ReadData(L"AmbientOcclusionVertex.cso");
	auto psBlob = DX::ReadData(L"AmbientOcclusionPixel.cso");

	device->CreateVertexShader(vsBlob.data(), vsBlob.size(), nullptr, m_AOVertex.ReleaseAndGetAddressOf());
	...
	device->CreateBlendState(&bd, m_AOBlend.ReleaseAndGetAddressOf());
}
...
void GameObjectManager::DrawAmbientOcclusion()
{
	...
	context->VSSetShader(m_AOVertex.Get(), NULL, 0);
